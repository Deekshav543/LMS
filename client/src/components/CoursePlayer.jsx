...
export default function CoursePlayer({ courseId }) {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      api.get(`/api/progress/summary?courseId=${encodeURIComponent(courseId)}`),
    ])
  ...
  const resolveYoutubeForLesson = async (lessonIdToResolve) => {
    ...
    setResolving(true)
    try {
      const res = await api.post('/api/lessons/youtube-url', { lessonId: lessonIdToResolve })
  ...
  const onCompleteActiveLesson = async () => {
    ...
    setCompleting(true)
    try {
      await api.post('/api/progress/complete', { userId: user.id, courseId, lessonId: activeLessonId })
      pushToast('success', '✅ Marked as complete!')

      const summaryRes = await api.get(`/api/progress/summary?courseId=${encodeURIComponent(courseId)}`)
